...
```{sql connection=con}
SELECT PNAME, FEE, ((FEE + 50) / 2.0) AS NOVA_TAXA FROM PRESERVE
```

4.  Apresente os valores médio, máximo e mínimo de ACRES para todas as reservas naturais localizadas no Arizona.

```{sql connection=con}
SELECT AVG(ACRES) AS MEDIA_ACRES, MAX(ACRES) AS MAXIMO_ACRES, MIN(ACRES) AS MINIMO_ACRES FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(PNAME) AS TOTAL_PNAME, COUNT(DISTINCT PNAME) AS PNAMES_DISTINTOS FROM PRESERVE
```
...
```{sql connection=con}
SELECT AVG(0.02 * ACRES) AS FEE_MEDIA FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(*) AS DEPARTAMENTO_20 FROM EMPLOYEE
WHERE DNO = 20
```
...
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS NUM_DEPARTAMENTOS FROM EMPLOYEE
```
