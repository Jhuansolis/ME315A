...
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE ORDER BY DNO, SALARY
```
Vemos que todas as querys resultam nos mesmo dados, porem estão ordenadas de forma diferente. O SELECT e o SELECT DISTINCT só retornam o mesmo número de linhas se não houver valores duplicados na sua tabela atual.
...
Sim, tempos pela lei distributiva C1 AND (C2 OR C3) = (C1 AND C2) OR (C1 AND C3)
...
Sim, novamente temos que C1 OR (C2 AND C3) = (C1 OR C2) AND (C1 OR C3)
...
A primeira query equivale à terceira query, pois a terceira traduz perfeitamente a conversão do AND original para OR e inverte os sinais relacionais (< vira >=, = vira <>).
...
As pimeira e a ultima são equivalentes por demorgan, mas a segunda não
